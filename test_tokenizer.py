...
import pytest
from tokenizer import Tokenizer, get_pair_counts, merge
import os
...
def test_counter():
    c = get_pair_counts([1,2,9,9,1,2])
    expected = Counter({(1, 2): 2, (2, 9): 1, (9, 9): 1, (9, 1): 1})
    assert c == expected

def test_merge():
    l = merge([1,2,9,9,1,2], (1,2), 256)
    expected = [256, 9, 9, 256]
    assert l == expected
...
def test_save_load():
    tokenizer = Tokenizer()
    tokenizer.train(train_text)
    tokenizer.save("tokenizer_test_tmp")
    ids = tokenizer.encode(val_text)
    tokenizer = Tokenizer()
    tokenizer.load("tokenizer_test_tmp.model")

    assert ids == tokenizer.encode(val_text)
    assert val_text == tokenizer.decode(tokenizer.encode(val_text))
    assert val_text == tokenizer.decode(ids)

    os.remove("tokenizer_test_tmp.model")
